...
class BPlusTree:
    ...
    def UnirIzquierda(self, temp, index):
        ...
        if len(temp.child)==1:
            ...
            if len(temp.child):
                if not len(temp.child[0].child):
                    temp.parent.child[index-1] = self.ReorganizarKeysHijo(temp.parent.child[index-1])
            else:
    ...
    def UnirDerecha(self, temp, index):
        ...
        if len(temp.child):
            if not len(temp.child[0].child):
                temp = self.ReorganizarKeysHijo(temp)
        else:
            temp = self.ReorganizarKeys(temp)
        return temp
    
    def MoverDerecha(self, temp, index):
        if len(temp.parent.child[index+1].child):
            if len(temp.parent.keys):
            ...
            temp.parent.child[index+1].keys.remove(val)
        if len(temp.child) and self.degree<5 or len(temp.child)==len(temp.keys):
    ...
    def MoverIzquierda(self, temp, index):
        ...
        if len(temp.parent.child[index-1].child):
            val = temp.parent.child[index-1].child[-1].keys[0]
            temp.parent.keys.pop(index-1)
    ...
    def graficar(self, database, table):
        ...
        f = open('prueba.dot','w',encoding='utf-8')
        ...
        os.system("dot -Tpng prueba.dot -o prueba.png")
        os.system("prueba.png")
